        ...
        if (isset($_POST['newadvertpublish']) || isset($_POST['newadvertnopublish'])) {
            ...
            if (!empty($_FILES)) {
                ...
                if (!in_array($fileType,  $types)) {
                    $msgerror['picture'] = 'extension ne correspond pas aux extensions acceptées';
                    $errors++;
                }
            }
            ...
            if (in_array($fileExtension, $extensionAllowed) && $size < $maxSize && $size !== 0) { //separer l'extension et le format + max taille
                ...
                if ($errors === 0) {
                    ...
                    move_uploaded_file($fileTemporyName, $fileDest); // deplacer dossier temporaire vers dossier final
